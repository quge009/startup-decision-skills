...
- `placeholders.json` — query values extracted from the candidate card
- `queries.json` — Tavily queries generated by `scripts/build_queries.py`
...
### Step 0 — Build uniform queries

Extract candidate, target market, sub-segment, incumbents, geographic scope,
specialty capability, and moat claim into `placeholders.json`; use `null` when
unknown. Then run:

```bash
python3 ~/.claude/skills/check-interpreter/scripts/build_queries.py \
  --working-dir <working_dir> \
  --placeholders-file <working_dir>/placeholders.json
```

The archetype is recorded for audit but never selects a query template. Resolve
any reported `<null:name>` placeholders before retrieval. See
`examples/example_placeholders.json` and `examples/example_queries.json`.
