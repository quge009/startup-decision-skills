...
Reference doc for the `market-check` skill. Defines the market-sizing
methodology — does the candidate
target a market large enough and healthy enough to support a viable
business?

All candidates use the same M-check methodology, Tavily query templates, and
verdict thresholds. No company-category classification is required.
...
- `market_queries.json` — Tavily query list generated by `scripts/build_queries.py`
- `tavily_m_results.json` — Tavily search results (executed by
  `market-check/scripts/run_tavily.py` using
...
Read `free_text.txt`, `candidate_card.md`, and `market_queries.json`. Extract
`founding_year` from the candidate card. It is required for time-bounded
retrieval (Step 2).

### Step 2 — Execute Tavily search (time-bounded)

Use `market-check/scripts/run_tavily.py` with parameters:

```
start_date = f"{founding_year - 3}-01-01"
end_date   = f"{founding_year}-12-31"
queries    = market_queries.json (generated by `scripts/build_queries.py`)
```
...
### Step 3a — Extract broad-market TAM (not candidate-claimed)

From Tavily results, identify the **broad-market TAM** for the
