...
Reference doc for the `moat-check` skill. Defines the **uniqueness / moat**
methodology — does the
candidate have defensible competitive advantage strong enough to sustain
captured market value?

All candidates audit their claimed moats through the same VRIO methodology.
There is no company-category-specific required moat type.
...
- `moat_queries.json` — Tavily query list generated by `scripts/build_queries.py`
- `tavily_u_results.json` — Tavily search results from
  `moat-check/scripts/run_tavily.py` (time-bounded window)
...
informational; the method does not prefer one moat type based on a company
category.
...
Read `free_text.txt`, `candidate_card.md`, and `moat_queries.json`. Extract
`founding_year` from the candidate card (required for time-bounded retrieval in
Step 3).
...
Use `moat-check/scripts/run_tavily.py` with parameters:

```
start_date = f"{founding_year - 3}-01-01"
end_date   = f"{founding_year}-12-31"
queries    = moat_queries.json (generated by `scripts/build_queries.py`)
```
